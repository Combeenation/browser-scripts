...
window.CbnRaygunExtensions = (function() {
	...
	function _decryptLog(verbose) {
		...
		try {
			...
			var sessionLogSpan = $('span.key:contains(sessionLog)').filter(function() { return $(this).text() === 'sessionLog'; });
				...
				log
					...
					.forEach(function(logEntry, idx, logEntries) {
						...
						encryptedDiv.append($('<p/>', {
							...
							text: $.format.date(logDate, 'HH:mm:ss.SSS') + ' ' + _getTimeDiffString(logDate, prevLogDate) + '   ' + logEntry.slmsg
						}));

						if (verbose && addDataArgs && (addDataArgs.length >= 2) && ('ChangeConfigurationValue' === addDataArgs[0])) {
							addDataArgs[1].forEach(function(arg, idx) {
								encryptedDiv.append($('<p/>', {
	 ...
	 */
	function _onXhrDone(data, textStatus, jqXHR ) {
	...
	function _updateShownDates() {
		...
		} else if (activeTab.hasClass('js-permanentlyignored-tab') || activeTab.hasClass('js-error-state-permanentlyignored')) {
		...
		errorData.records.forEach(function(record, idx) {
